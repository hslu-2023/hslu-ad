...
public final class FindFileTask extends CountedCompleter<String> {
    ...
    public void compute() {
        ...
        if (list != null) {
            for (File file : list) {
                if (file.isDirectory()) {
                    this.addToPendingCount(getPendingCount() + 1); // prepare framework for new task
                    new FindFileTask(this, regex, file, result).fork(); // start new task asynchronous
                } else if (regex.equalsIgnoreCase(file.getName()) && result.compareAndSet(INITIAL_RESULT_VALUE, file.getParent())) {
                    this.quietlyCompleteRoot(); // send completed signal to root
                    break;
                }
            }
        }
        tryComplete(); // end current task
    }
